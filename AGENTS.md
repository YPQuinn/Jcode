...
- `coding-agent` 第二阶段实施中，计划位于 `docs/plans/coding-agent-phase-2-local-tools.md`。PR 1 已完成工具枚举/配置、产品 policy、Session 显式装配、旧 Config 的 read-only 兼容和 prompt 同源；当前真实工具仍只有 `read`。Bash 配置拒绝隐式初始化环境变量且最大 timeout 硬限制为 3600 秒，授权请求诊断不暴露参数或工作目录。后续先完成显式启用的 write/edit/bash 闭环，再补 grep/find/ls；继续遵守 mutation 整批顺序执行、原子文件替换、有界进程输出与显式 ripgrep 后端约束。不得把后续计划中的工具/API 视为当前已具备能力。

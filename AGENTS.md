...
└── docs/
    ...
    ├── plans/           # 方案文档
    │   └── archived/    # 已实现或已归档的过往方案；包含 coding-agent 阶段一至三实施记录
...
- `coding-agent` 第二阶段已完成，实施记录归档于 `docs/plans/archived/coding-agent-phase-2-local-tools.md`。旧 Config 与 `tools == null` 默认仍只有 `read`，显式 `coding(BashConfig)` profile 提供 `read/write/edit/bash`，`codingWithSearch(BashConfig, SearchConfig)` 显式提供全部七个工具。文件 mutation 以 8 MiB 为原始/最终边界，保留 symlink/`..` 原生解析、UTF-8 BOM、未触及文本及已有 POSIX 权限，提交仅用同目录临时文件加原子替换。Bash 使用绝对 executable、`--noprofile --norc`、完全替换的环境快照和每 Session 最多 4 个进程；合并输出持续 drain 并仅保留 2000 行/50 KiB tail，timeout/cancel 终止进程树，Session close 回收活跃进程。`write`/`edit`/`bash` 均为 `SEQUENTIAL`，输出采集不受慢 update sink 阻塞且已接纳 update 在完成前 settle。`ls` 使用 `DirectoryStream` 与有界 top-N heap；`grep/find` 共享显式配置并经 capability probe 的 ripgrep 14+ backend，独立 drain stdout/stderr，以有界 NDJSON/NUL parser 处理结构化结果，正向 glob 仅后置过滤 ignore-aware 结果。搜索单文件上限 8 MiB、单 record 上限 64 KiB、最多扫描 100000 条记录，特殊路径采用 JSON 字符串，达到结果/字节预算时内部取消子进程。PR 6 已补齐全工具 fake-model Git 工作区闭环、可恢复故障矩阵、进程/临时文件清理证据和严格 `local-tools-smoke` profile；审查修复后全仓 721 个测试通过，其中本模块 181 个，严格 native smoke 通过。
- `coding-agent` 第二阶段审查修复：进程树终止跨 graceful/forceful 阶段保留已观察后代句柄，父进程退出不提前结算；终止有界，runner 自有调度资源在已接纳执行结束后释放。publisher 失败立即取消工具自有进程 signal，清理后传播原基础设施异常。grep 显式文件启动前检查 8 MiB 上限，文件操作数统一加 `./` 防止 `-` 被当 stdin；glob 的 `*` 分支优先于字面字符比较，正确匹配含星号的合法文件名。
- `coding-agent` 第三阶段已完成，实施记录归档于 `docs/plans/archived/coding-agent-phase-3-project-context.md`。新增显式启用且有界的物理祖先/全局项目指令发现、不可变来源与诊断快照、安全可逆的 `<project_context>` Prompt 装配，以及只在 idle 接纳的原子 reload。候选仅 `AGENTS.override.md`、`AGENTS.md`、`AGENTS.MD`，不探测或回退到 CLAUDE 文件；旧配置默认不新增发现 I/O。loader 处理严格 UTF-8、BOM、symlink/hardlink 去重、受验证的嵌套 linked-worktree 根遮蔽及固定资源预算；失败或取消 reload 不替换旧快照。`agent-core` 的 `updateSystemPrompt()` 与 run/close 共用接纳边界，只更新 prompt 和公开 context/state，保留历史、队列与工具。审查修复后，reload 在锁内释放接纳、锁外完成 stage，成功/失败/取消回调均可重入；close 超时不伪造仍阻塞的加载已结束。来源打开前复核普通文件属性；保留预算只计验证成功的来源，实际读取预算仍计失败读取；路径可渲染性与损坏 `commondir` 按 failureMode 处理。最终全仓 `mvn clean verify` 共 762 个测试，其中 `coding-agent` 220 个，0 failure/error/skipped；严格 `local-tools-smoke` 与生命周期/加载边界连续三轮复验通过。

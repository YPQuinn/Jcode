...
> **历史记录说明（2026-09-20）**：本轮已按 pi 的实际工具行为调整第二阶段契约，当前规范见 [本地工具行为对齐记录](local-tools-behavior-alignment.md)。下文保留原始实施和审查证据，不再要求旧文件 CAS/原子替换、严格无容错编辑、Bash 初始化变量禁令/3600 秒上限、自研后置 glob、rg 14+ handshake 或单文件 8 MiB 搜索限制。进程清理、背压、policy 和模块边界不变。
